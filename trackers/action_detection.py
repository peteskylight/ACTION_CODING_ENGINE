...
class ActionDetectionThread(QThread):
    ...
    def __init__(self,main_window, video_keypoints, black_frames, video_frames, detections):
        super().__init__()
 ...
 # Recompile model (optional but can help)
        ...
        # self.actions_list = np.array(['LEFT ARM EXTENDING SIDEWARDS', 'RIGHT ARM EXTENDING SIDEWARDS', 'FACING DOWNWARDS', 'FACING LEFT', 'FACING RIGHT', 'SITTING', 'STANDING'])
        self.actions_list = self.main_window.actions_list
    ...
    def predict_actions(self, frames_keypoints):
        ...
        for person_id, keypoints_sequence in frames_keypoints.items():
            ...
            if keypoints_array.shape != (self.buffer_size, 34):
                ...
                predictions[person_id] = "UNKNOWN ACTION"
                continue
            ...
            try:
                ...
                predictions[person_id] = "UNKNOWN ACTION"  # Handle prediction errors gracefully
